...
namespace PLC_POSITION.VIEW
{
    public partial class RealTimeChart : Form
    {
        ...
        {
            string queryString = string.Format("select * from MAT_Chemical_Line.dbo.Simple_RecordTable where TypeId={0} and RecordTime >= '{1}' and RecordTime <='{2}'", typeid, DateTime.Now.ToString("yyyy-MM-dd"),DateTime.Now.AddDays(1).ToString("yy-MM-dd"));
        ...
        {
            //填入必填项
            if (comboBox1.Text == "" || comboBox2.Text == "" || comboBox3.Text == ""||comboBox5.Text == ""||textBox1.Text==""||textBox2.Text=="")
            {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            Series series = chart1.Series[0];
            GetDataFromSql();
            series.Points.AddXY(DateTime.Now, float_temp[selectIndex]);
            double sumPoints = 0;
            for (int i = 0; i < series.Points.Count; i++)
            {

                sumPoints += series.Points[i].YValues[0];
            }
            double averagePoint = sumPoints/(series.Points.Count);
